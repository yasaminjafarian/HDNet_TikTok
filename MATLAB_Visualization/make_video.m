...
for i = 1:2
    ...
    d = dlmread([pred_path,name,'.txt'])-1.5;
    ...
    mkdir([pred_path,'video/',name,'/',color_name,'/'])
    ims1 =CaptureFigVid([azs,als], [pred_path,'video/',name,'/',color_name,'/'],name,color_name,OptionZ);
end
delete TEST.jpg
%% make the combined gray and color images
d = dir([pred_path,'video/',name,'/','color','/*.jpg']);
mkdir([pred_path,'video/',name,'/','gray_color'])
for i =  1:length(d)
    im1 = imread([pred_path,'video/',name,'/','gray','/',int2str(i),'.jpg']);
    im2 = imread([pred_path,'video/',name,'/','color','/',int2str(i),'.jpg']);
    im = [im1,im2];
    imwrite(im,[pred_path,'video/',name,'/','gray_color/',int2str(i),'.jpg'])
end

%% convert the image sequence to video.
workingDir = [pred_path,'video/',name,'/','gray_color','/'];
out_dir = [pred_path,'video/',name,'/'];
...
if remove_images  
    rmdir([pred_path,'video/',name,'/','gray_color'],'s')
    rmdir([pred_path,'video/',name,'/','gray'],'s')
    rmdir([pred_path,'video/',name,'/','color'],'s')
end
toc
